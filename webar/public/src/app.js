    ...
    async () => {
      try {
        ...
        try {
          ...
          // Хэрэглэгч зөвшөөрөөгүй, эсвэл GPS уншиж чадсангүй
          const msg =
            e?.code === 1
          ...
          (otpError || $msg).textContent = msg; // аль UI-гаа ашигла
          setTimeout(() => {
            (otpError || $msg).textContent = "";
          }, 3500);
          return;
        }

        // 2) Координатыг шалгаад тоо болгон хувиргана
        const { latitude, longitude, accuracy } = loc.coords || {};
        if (typeof latitude !== "number" || typeof longitude !== "number") {
          (otpError || $msg).textContent =
            "Байршил буруу байна. Дахин оролдоно уу.";
          setTimeout(() => {
            (otpError || $msg).textContent = "";
          }, 3500);
          return;
...
tapLay.addEventListener("pointerdown", async () => {
  ...
  try {
    if (window.__introStarted) await startIntroFlow(true);
  } catch (e) {
...
async function startIntroFlow(fromTap = false) {
  ...
  if (isIOS) {
    ...
    if (vIntro.readyState >= 1) fitPlaneToVideo(vIntro);
      ...
      vIntro.addEventListener("loadedmetadata", () => fitPlaneToVideo(vIntro), {
        once: true,
      });
  }
